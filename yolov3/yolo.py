...
class YOLO(object):
    _defaults = {
        "model_path"        : 'logs/Epoch100-Total_Loss0.6870-Val_Loss1.1615.pth',
    ...
    def detect_image(self, image):
        ...
        #   也可以直接resize进行识别   letterbox_image函数,加灰条使其成为正方形图像
        #---------------------------------------------------------#
        ...
        photo = np.transpose(photo, (2, 0, 1))   #通道调整，pytorch规定通道数在前
        #---------------------------------------------------------#
        #   添加上batch_size维度
        #---------------------------------------------------------#
        images = [photo]    #扩充维度
        ...
        with torch.no_grad():
            ...
            #---------------------------------------------------------#
            try :
                batch_detections = batch_detections[0].cpu().numpy()         #等同if len(batch_detection)<=0  return image
            except:
                return image
            ...
            #   对预测框进行得分筛选
            #---------------------------------------------------------#
            top_index   = batch_detections[:, 4] * batch_detections[:, 5] > self.confidence    #框的置信度和种类的置信度相乘进行置信度的判断
            ...
            #   我们需要对其进行修改，去除灰条的部分。   yolo_correct_box 去除灰条
            #-----------------------------------------------------------------#
            if self.letterbox_image:
