...
def fit_one_epoch(net, yolo_loss, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda):
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            outputs.shape 
            0 (1,27,20,20)  batch_size = 1 ,27为通道数 （4类） 包含3*（5+num_classes）， 20*20为特征图尺寸
            1 (1,27,40,40)
            2 (1,27,80,80)
            '''
            losses      = []
...
if __name__ == "__main__":
    ...
    if True:
        ...
        gen_val         = DataLoader(val_dataset, shuffle=False, batch_size=Batch_size, num_workers=0, pin_memory=True,
                                    drop_last=True, collate_fn=yolo_dataset_collate)
        # 查看批次图片
        ...
        # 或者gen_iter = iter(gen)
        # img,label = gen_iter.next()

        # for i_batch, img in enumerate(gen):
        #     if i_batch == 0:
        #         print('img[1]:',img[1])
        #         #print(img[0].shape) (4, 3, 640, 640)
        #         fig = plt.figure()
        #         grid = utils.make_grid(torch.tensor(img[0]))
        #         #print(grid.shape)  torch.Size([3, 644, 2570])
